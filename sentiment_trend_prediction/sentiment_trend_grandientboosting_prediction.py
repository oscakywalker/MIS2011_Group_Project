...
DEFAULT_OUTPUT = SCRIPT_DIR / "sentiment_trend_grandientboosting_prediction.html"
DEFAULT_FUTURE_OUTPUT = SCRIPT_DIR / "sentiment_trend_grandientboosting_prediction_future.html"
DEFAULT_GRANULARITY = "month"
TRAIN_RATIO = 0.8
DEFAULT_WINDOW = 4
DEFAULT_ESTIMATORS = 160
DEFAULT_MAX_DEPTH = 3
...
def create_regressor(n_estimators, max_depth):
    ...
    return (
        ...
        "sklearn_gbr_fallback",
    )


def fit_xgboost_series(train_series, full_series, test_length, window_size, n_estimators, max_depth):
    ...
    if len(train_series) <= 1:
        ...
        return {
            ...
            "predicted_all": [None] * len(train_series) + fallback,
            "window_size": 1,
            "estimators": 0,
            "max_depth": max_depth,
            "train_rmse": None,
            "backend": "fallback_constant",
        }

    if len(train_series) <= actual_window:
        actual_window = max(1, len(train_series) - 1)

    feature_train, target_train = build_training_samples(train_series, actual_window)
    ...
    model, backend = create_regressor(n_estimators, max_depth)
    model.fit(feature_train, target_train)
    train_pred = model.predict(feature_train)
    train_rmse = math.sqrt(np.mean((train_pred - target_train) ** 2))

    history = list(full_series[:len(train_series)])
    predicted_test = []
    for step in range(test_length):
        feature = np.array(history[-actual_window:], dtype=np.float32).reshape(1, -1)
        prediction = clamp_percentage(float(model.predict(feature)[0]))
...
def forecast_xgboost_series(full_series, forecast_steps, window_size, n_estimators, max_depth):
    actual_window = choose_window_size(len(full_series), window_size)
    if len(full_series) <= 1:
        return {"predicted_future": [full_series[-1]] * forecast_steps, "backend": "fallback_constant"}

    if len(full_series) <= actual_window:
        actual_window = max(1, len(full_series) - 1)

    feature_train, target_train = build_training_samples(full_series, actual_window)
    if len(feature_train) == 0:
        return {"predicted_future": [full_series[-1]] * forecast_steps, "backend": "fallback_constant"}
    ...
    return {"predicted_future": predicted_future, "backend": backend}
...
def build_prediction_figure(rows, train_rows, test_rows, model_results, backend_name):
    ...
    for polarity in POLARITIES:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    "%{x}<br>Grandient Boosting 趋势预测<br>"
                    + POLARITY_LABELS[polarity]
        ...
        summary_rows.append(
            [
                ...
                str(model_results[polarity]["estimators"]),
                str(model_results[polarity]["max_depth"]),
                format_stat(model_results[polarity]["train_rmse"], 4),
                f"{mae:.2f}%",
                f"{rmse:.2f}%",
            ]
        )
    ...
    fig.add_trace(
        go.Table(
            header=dict(
                values=["情感类别", "窗口长度", "树数量", "最大深度", "训练 RMSE", "测试 MAE", "测试 RMSE"],
                fill_color="#F1F5F9",
                line_color="#D8DEE9",
                align="center",
    ...
    fig.update_layout(
        title=dict(text="情感趋势 Grandient Boosting 预测", x=0.02, y=0.98, font=dict(size=28, color="#30333A")),
...
def build_future_prediction_figure(rows, future_days, future_predictions, granularity: str, backend_name: str):
    labels = [row["label"] for row in rows]
...
def parse_args():
    parser = argparse.ArgumentParser(description="Predict sentiment trend with Grandient Boosting-style tree models.")
    parser.add_argument(
        "--input",
        nargs="+",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Look-back window size for each tree model sequence.",
    )
    parser.add_argument(
        "--estimators",
        type=int,
        default=DEFAULT_ESTIMATORS,
        help="Number of boosting estimators.",
    )
    parser.add_argument(
        "--max-depth",
        type=int,
...
def main():
    ...
    fig = build_prediction_figure(rows, train_rows, test_rows, model_results, backend_name)
    ...
    future_predictions = {}
    future_backend_name = None
    for polarity in POLARITIES:
        future_result = forecast_xgboost_series([row[polarity] for row in rows], len(future_days), args.window_size, args.estimators, args.max_depth)
        future_predictions[polarity] = future_result["predicted_future"]
        future_backend_name = future_result["backend"]
    future_fig = build_future_prediction_figure(rows, future_days, future_predictions, args.granularity, future_backend_name)
