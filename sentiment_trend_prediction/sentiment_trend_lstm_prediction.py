...
DEFAULT_OUTPUT = SCRIPT_DIR / "sentiment_trend_lstm_prediction.html"
DEFAULT_FUTURE_OUTPUT = SCRIPT_DIR / "sentiment_trend_lstm_prediction_future.html"
...
def fit_lstm_series(train_series, full_series, test_length, window_size, epochs, hidden_size):
...
def forecast_lstm_series(full_series, forecast_steps, window_size, epochs, hidden_size):
...
def build_prediction_figure(rows, train_rows, test_rows, model_results):
    ...
    fig.update_layout(
        title=dict(text="情感趋势 LSTM 预测", x=0.02, y=0.98, font=dict(size=28, color="#30333A")),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Predict sentiment trend with LSTM models.")
    parser.add_argument(
        "--input",
        nargs="+",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Look-back window size for each LSTM sequence.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
...
def main():
    ...
    future_predictions = {}
    for polarity in POLARITIES:
        future_predictions[polarity] = forecast_lstm_series([row[polarity] for row in rows], len(future_days), args.window_size, args.epochs, args.hidden_size)["predicted_future"]
